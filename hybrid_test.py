...
_NON_RETRYABLE_ERROR_PATTERNS = re.compile(
    r"(cuda|out of memory|oom|accelerator error|acceleratorerror|"
    r"cudnn|cublas|device-side assert|"
    r"timed?[ _]?out|timeout|"
    r"connection refused|connection error|connection reset|"
    r"econnrefused|econnreset|broken pipe|"
    r"unreachable|no route to host|name or service not known|"
    r"dns|network is unreachable|"
    r"resource exhausted|memory error|memoryerror)",
    re.IGNORECASE,
)


def _is_retryable_failure(error_message: Optional[str]) -> bool:
    """Distinguishes resource/infrastructure-class failures (GPU OOM,
    device errors, timeouts, connection failures) from data/query-shape
    failures (bad SQL, unknown column, no matching documents, etc.).

    Cross-branch entity/filter reuse can plausibly fix the latter (a
    better-informed retry may resolve an ambiguous column or widen a
    query) but can never fix the former -- retrying an out-of-memory or
    connection failure with different entities just wastes latency and
    guarantees a second identical failure. Retries are skipped for the
    former and still attempted for the latter.
    """
    if not error_message:
        return True
    return not _NON_RETRYABLE_ERROR_PATTERNS.search(error_message)
...
class HybridAgent:
    ...
    async def _execute(
        ...
        structured_result: Optional[StructuredAgentResult] = None
        semantic_result: Optional[SemanticAgentResult] = None
        # Track the specific error message per branch (not just the
        # accumulated `errors` list) so the retry gate below can
        # classify *which* branch failed *why*, independent of
        # whatever else has been appended to `errors` in the meantime.
        structured_err_msg: Optional[str] = None
        semantic_err_msg: Optional[str] = None

        if routing.mode is RoutingMode.STRUCTURED_ONLY:
            structured_result, structured_err_msg = await self._run_structured(query, conversation)
            if structured_err_msg:
                errors.append(structured_err_msg)

        elif routing.mode is RoutingMode.SEMANTIC_ONLY:
            semantic_result, semantic_err_msg = await self._run_semantic(query, conversation)
            if semantic_err_msg:
                errors.append(semantic_err_msg)
            ...
            semantic_result, semantic_err_msg = await self._run_semantic(
            ...
            (structured_result, structured_err_msg), (semantic_result, semantic_err_msg) = await asyncio.gather(
        ...
        # discovered by the successful branch. Skipped entirely when
        # the failure is a resource/infrastructure error (GPU OOM,
        # timeout, connection failure) that no amount of extra
        # entities/filters could fix -- see `_is_retryable_failure`. ----
        if self.config.enable_retry and self.config.max_branch_retries > 0:
            ...
            if structured_attempted and structured_failed and semantic_result is not None and semantic_result.success:
                if not _is_retryable_failure(structured_err_msg):
                    retries.append({
                        "branch": "structured",
                        "reused_from": "semantic",
                        "skipped": True,
                        "reason": f"non_retryable_infrastructure_error: {structured_err_msg}",
                    })
                else:
                    discovered = _entities_from_records(semantic_result.chunks)
                    if discovered:
                        retried_result, retry_err = await self._run_structured(
            ...
            if semantic_attempted and semantic_failed and structured_result is not None and structured_result.success:
                if not _is_retryable_failure(semantic_err_msg):
                    retries.append({
                        "branch": "semantic",
                        "reused_from": "structured",
                        "skipped": True,
                        "reason": f"non_retryable_infrastructure_error: {semantic_err_msg}",
                    })
                else:
                    discovered = _entities_from_records(structured_result.rows)
                    if discovered:
                        retried_result, retry_err = await self._run_semantic(
                            query, conversation, extra_entities=discovered,
                        )
                        retries.append({
                            "branch": "semantic",
